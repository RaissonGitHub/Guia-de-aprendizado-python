...
Elas são particularmente úteis em situações onde você precisa de uma pequena função para uma única operação, muitas vezes como argumento para outra função de ordem superior (funções que aceitam outras funções como argumentos, como m`ap(), filter() ou sorted()`).
